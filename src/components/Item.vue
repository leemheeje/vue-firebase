...
<div class="items">
    <div class="itin">
        <div class="uinf">
            <a href="javascript:;" @click="itemDetailView" class="thub" :style=" `background-image: url(${item_info.item_thumb[0]});` ">
                <div class="fblb" v-if="!isMypage">
                    <span class="flb tp0"> 
                ...
                <div class="mblb" v-if="item_info.item_thumb.length > 1">
                    <span class="t">{{ item_info.item_thumb.length - 1 }}</span>
                </div>
            </a>
            <div class="txts">
                <div class="txtop">
                    <a href="javascript:;" @click="userMypageView" class="u_thub" :style=" `background-image: url(${item_info.item_user_thumb});` "></a>
                    <span class="tt">{{ item_info.item_title }}</span>
                    <a href="javascript:;" @click="userMypageView" class="st">{{ item_info.item_intro }}</a > 
                    <div class="fblb" v-if="isMypage">
                        <span class="flb tp0"> 
                            <i class="fas fa-eye"></i> 
                            <span class="t">{{ item_info.item_view }}</span>
                    ...
                    <a href="javascript:;" @click="itemDetailView" class="sts" >{{ item_info.item_user_nm }}</a >
                    <a href="javascript:;" @click="itemDetailView" class="dt" >{{ item_info.item_create_date | date_after_day }}</a > 
                </div> 
            </div>
        </div>
    </div>
...
export default {
    props: ["item_user_uid", "item_id", 'isMypage'],
    data() {
        return { modalShow: false, add_view: 0 };
    },
    created() {},
    computed: {
        ...
        itemDetailView(s) {
            let chkFavorite = () => {
                ...
                if (this.isAuth && this.user.uid && this.item_info) {
                    ars = this.item_info.item_favorite_group.filter(ar => ar == this.user.uid);
                }
